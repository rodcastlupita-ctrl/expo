 ...
 *
 * RN core matches `fontWeight` and `fontStyle` against them — but they are not names an app chose, so reporting them would claim
 * them in the namespace that ``FontLoaderModule/loadAsync`` loads into.
 */
